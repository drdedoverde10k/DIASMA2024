...
# Registro del inicio
TimeStart="$(date +"%c")"
echo "Starting of the scipt $TimeStart" > celoss_start_log.txt

# Activar el entorno de Conda
source /home/fespinoza/miniconda3/etc/profile.d/conda.sh
conda activate master_nn_training

# Ejecutar el script Python
python training_script_celoss.py || { echo "Error en el script Python"; exit 1; }

# Registro del fin
TimeEnd="$(date +"%c")"
echo "Ending script at $TimeEnd" >> celoss_end_log.txt
